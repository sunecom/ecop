...
所有页面及业务 API 使用 Basic Auth；仅 `/healthz` 返回无敏感内容的存活状态。计算 POST 另验证同源 Origin 和页面 nonce。MCP token 不传至浏览器，MCP 不映射宿主端口、无公网路由。Web 宿主端口仅监听 `127.0.0.1:18765`，必须由服务器现有反向代理提供 HTTPS。所有登录账户共享同一演示空间。
