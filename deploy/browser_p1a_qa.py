"""Browser acceptance for the isolated P1A candidate using a real Chromium binary."""
import argparse
import json
import os
import time
...
def main():
    ...
    if parsed.port != 18766:
        raise ValueError('Browser QA requires the isolated loopback candidate on port 18766')
    ...
    with sync_playwright() as playwright:
        ...
        page.wait_for_function("document.getElementById('countLive').textContent === '6'")
        if page.locator('.workbench-tab').count() != 6 or page.locator('.calc-form').count() != 6:
            raise AssertionError('Expected six workbench tabs and forms')
        ...
        page.screenshot(path=str(output_dir / 'p1a-desktop-1440.png'), full_page=True)
        ...
        mixer_base = run_module('mixer')
        run_module('splitter')
        valve_base = run_module('valve')
        mixer_disturb = run_module('mixer', {
            'feed1_flow_kg_h': 700,
            'feed1_temperature_C': 20,
            'feed1_pressure_kPa': 300,
            'feed2_flow_kg_h': 300,
            'feed2_temperature_C': 80,
            'feed2_pressure_kPa': 300,
        })
        if mixer_base['run_id'] == mixer_disturb['run_id']:
            raise AssertionError('Mixer history did not preserve separate runs')
        ...
        if downloaded['run_id'] != mixer_disturb['run_id']:
            raise AssertionError('Downloaded record is not the latest UI result')
        downloads.append(str(download_path))
        page.screenshot(path=str(output_dir / 'p1a-mixer-result-1440.png'), full_page=True)
        ...
        page.screenshot(path=str(output_dir / 'p1a-mobile-390.png'))
        if not page.locator('.calc-form[data-module="mixer"]').is_visible():
            raise AssertionError('Mixer form is not visible at 390px')
    ...
    evidence = {
        ...
        'valve_run_id': valve_base['run_id'],
    }
    output = output_dir / 'p1a-browser-evidence.json'
