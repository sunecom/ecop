...
- 引擎启动时间：`2026-09-18T15:47:33.968731569Z`。
- 直连探测脚本：`deploy/probe_p2_schema.py`；仅接受 `http://127.0.0.1:15904/mcp`。
- 原始证据：`.local/qa/P2/p2-schema.json`，SHA-256 `5fd564263dc41353f048cd8b8245d781b41b416f58b332b31d5489a45b24d5b0`。
...
- `dwsim_thermo_add_compounds` 的 `added`、`dwsim_thermo_set_property_package` 的 `property_package` 均逐次核对；不依赖无法正确显示物性包的 flowsheet 摘要字段。
...
- 两相字段的实际舍入/求解残差在 50/50 水–乙醇 NRTL 工况为最大 `0.011691126 kg/h`，即总流量的 `1.1691e-5`；候选预设相重组相对上限 `2e-5`（20 ppm）。故障回放覆盖“整体组成不变但相分配错误”，必须被拒绝。
...
| Water/Ethanol 50/50 mass% | NRTL | 86.134311 | 0.499998 | 0.592795 | 混合物指定摩尔汽相率闪蒸温度 |
| Water/Ethanol 50/50 mass% | Raoult | 93.986695 | 0.500002 | 0.542227 | 理想溶液对照温度 |

NRTL 与 Raoult 相差 `7.852384 °C`，只证明物性包选择确实作用于模型，**不构成准确性验证**。
...
- 水–乙醇未完成实验数据或桌面同模型交叉验证；当前仅为引擎一致性层级。MCP 未暴露 NRTL 二元参数来源和有效温区，因此明确记录为未知，不因收敛而推断适用。
